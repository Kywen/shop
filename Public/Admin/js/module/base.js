...
YX.fn.showMe = function () {
    ...
    // alert(this.$userPic.attr('src')+"");
    this.$userPic.attr('src', getAvatar(user.avatar))
    if(this.$userPic.attr('src') == ""){
        this.$userPic.attr('src','http://139.199.198.151:8000/shop_img/images/default-icon.png');            
    }
    // alert(this.$userPic.attr('src')+"")
...
YX.fn.openChatBox = function (account, scene) {
    ...
    if(scene=="p2p"){
        ...
        if(info.account == userUID){
            ...
            this.$chatTitle.find('img').attr('src', "http://139.199.198.151:8000/shop_img/images/myPhone.png") 
        }else{
        ...
        case "session":   
            ...
            if(scene==="p2p"){
                ...
                if(msg.target === userUID){
                    info.nick = "我的手机"
                    info.avatar = "http://139.199.198.151:8000/shop_img/images/myPhone.png"
                }else{
